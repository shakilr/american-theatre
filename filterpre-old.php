...
<style>
	...
	.row.not_empty.item_3, .row.not_empty.item_4 , .row.not_empty.item_8,.row.not_empty.item_6, .row.not_empty.item_21 {    display: none !important;}
</style>
<?php
...
foreach($getStates as $key => $states){
	...
	$getCompanyDetails_q = 'SELECT meta_key,meta_value from `wp_postmeta` where `meta_key` = "companys_name" and post_id  = "'.$post_id.'"';
	$getCompanyDetails = $wpdb->get_results($getCompanyDetails_q);
...
$n++;
	...
	<div class="row not_empty item_<?php echo $n;?> <?php $v?>">
		<div class="col-md-12">
...
<script>
	jQuery(document).ready(function($) {
		...
		jQuery('.row.not_empty.item_3, .row.not_empty.item_4 , .row.not_empty.item_8, .row.not_empty.item_21').remove();
