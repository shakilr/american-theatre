...
#bishop_arts_theatre_center_, #the_invisible_theatre,#bay_area_film_mixer,#su_teatro_,#denver_center_for_the_performing_arts_theatre_company, #shattered_globe_theatre,#children's_theatre_company_, #an_axe-mas_story_the_musical_,#irish_repertorytheatre,#manhattan_theatre_club_,#ping_chong_&_company, #the_cherry_arts_,#brian_sanders'_junk, #bryn_mawr_college, #cooper_jordan_entertainment_, #stage_west_theatre {display: none !important;} 
</style>
<?php
...
foreach($getStates as $key => $states){
    ...
    $dq = 'SELECT m.meta_key,m.meta_value from `wp_postmeta` m INNER join wp_posts p on p.ID = m.post_id
	where  `meta_key` = "start_date" and  post_id  = "'.$post_id.'"  ';
 
	$getdate = $wpdb->get_results($dq);
	$s_date = date("Y-m-d",strtotime($getdate[0]->meta_value));
